...
from .forms import CategoryForm,AreaForm,PayMethodForm,HolidayForm,RestaurantForm,ReviewForm,ReservationForm,CompanyForm,FavoriteForm,RestaurantCategorySearchForm,YearMonthForm,ProfileForm
import stripe
...
class CheckoutView(LoginRequiredMixin,View):
    def post(self, request, *args, **kwargs):
        ...
        #return redirect(checkout_session.url['url'])
...
class FavoriteView(PremiumMemberMixin,View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "お気に入り登録が完了しました")
...
class FavoriteDeleteView(PremiumMemberMixin, View):
    def post(self, request, pk,  *args, **kwargs):
        ...
        favorite.delete()
        messages.success(request, "お気に入りから削除が完了しました")
        return redirect("nagoyameshi:favorite")
...
class ReservationDeleteView(PremiumMemberMixin,View):
    def post(self, request, pk,  *args, **kwargs):
        ...
        if now < deadline:
            print("予約キャンセル")
            reservation.delete()
            messages.success(request, "予約キャンセルが完了しました")
        else:
            print("予約キャンセルできません。")
            messages.error(request, "予約キャンセルに失敗しました")
        return redirect("nagoyameshi:reservation")
